...
from module.charts import chart
import os
import csv
import requests
...
def run():
    ...
    while scraping:
        ...
        print(f"You have chosen {chart[number]} and data will be saved in .csv file")
        ...
        match number:
            ...
            case 6:
                save_data(
                    ...
                    WebScraping(
                        "https://www.imdb.com/chart/top-english-movies?pf_rd_m=A2FGELUUNOQJNL&pf_rd_p"
                        "=470df400-70d9-4f35-bb05-8646a1195842&pf_rd_r=0APT2MECQACXEH9AG2TB&pf_rd_s"
                        "=right-4&pf_rd_t=15506&pf_rd_i=bottom&ref_=chtbtm_ql_4"
                    ).get_imdb(),
                )
            case 7:
                exit(0)
        sleep(1)
        more_scraping = input("All done. Do you want to scrap another one? [y/n] ")
